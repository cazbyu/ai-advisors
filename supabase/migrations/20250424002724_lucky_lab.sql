/*
  # Project-Specific Document Management System

  1. New Tables
    - `0003_document_index`
      - `id` (uuid, primary key)
      - `project_id` (text, default '0003')
  ...
  2. Security
    - Enable RLS on `0003_document_index` table
    - Add policies for authenticated users to manage documents
    - Add policy for public read access to public documents
*/

CREATE TABLE IF NOT EXISTS "0003_document_index" (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  project_id text NOT NULL DEFAULT '0003',
  advisor text REFERENCES advisors(id),
  topic text NOT NULL,
  title text NOT NULL,
...
ALTER TABLE "0003_document_index" ENABLE ROW LEVEL SECURITY;
...
CREATE TRIGGER update_0003_document_index_updated_at
  BEFORE UPDATE ON "0003_document_index"
  FOR EACH ROW
